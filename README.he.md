...
**כלי שורת פקודה ידידותי-סוכן (agent-native) ל-[morning](https://www.greeninvoice.co.il)** — מערכת הנהלת החשבונות המובילה בישראל.
...
אני מאור, מפתח עצמאי שמנהל את [JangoAI](https://jango-ai.com) — סטודיו קטן לאוטומציה ו-AI בישראל. אני עובד עם ה-API של morning כל יום — בעסק שלי ובפרויקטים של לקוחות — ונמאס לי לכתוב את אותו boilerplate של `httpx` + JWT בכל פרויקט מחדש.

אז בניתי CLI סדור מסביב לזה, ואין סיבה לשמור אותו לעצמי. אם גם אתם עובדים עם ה-API של morning, הכלי הזה יחסוך לכם זמן. אני משתף אותו עם הקהילה הישראלית כי אני חושב שמגיע לנו כלים טובים יותר לפלטפורמות המקומיות שלנו.

## מה זה

`morning-cli` עוטף את כל ה-API של morning ב-CLI נקי שאפשר להשתמש בו משורת הפקודה, מסקריפטים, או מתוך סוכני AI (Claude Code, Cursor, וכו'). הכלי לא מממש חשבוניות בפייתון — הוא קורא ישר ל-API האמיתי של morning.
